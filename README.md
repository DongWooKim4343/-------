...
데이터 수집을 위해 실험 대상자의 턱걸이 기록을 직접 측정하였다.\
...
키, 몸무게, BMI지수는 총 91개의 데이터가 수집 되었으며, 인바디 검사 기계의 제한적인 보급 장소의 특성상 인바디 점수는 48개의 데이터만 수집되었다.
...
키, 몸무게, BMI수치는 턱걸이와는 큰 상관이 없는 것으로 보인다, 일반적으로 생각할 때 몸무게가 가벼우면 턱걸이를 더 많이할 것이라고 생각한다. 하지만 값을 볼때 큰 상관이 없다. 

몸무게가 많이 나가더라도 근육량이 많으면 턱걸이를 더 잘하기 때문이라고 예상.  
...
또한 인바디 점수의 표준오차는 5.7회 정도이며 다른 3가지 건강지표보다 오차가 더 적은편에 속합다.
...
위의 스포츠 관련된 인물 4명은 BMI기준 모두 과체중~고도비만에 속한다. 
...
## 마치며
1. 키와 체중의 비율을 계산한 BMI 지수는 본 연구의 결과로는 아무런 의미가 없는 수치일 뿐이다. 
...
4. 이 연구의 결과로 볼 때, 운동시 체지방은 감소시키고 근육량을 키워야 바람직하다.
